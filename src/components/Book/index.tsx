...
const Book: React.FC<dates> = ({date, handleClick}) => {
  ...
  return (
    <>
      ...
      >
        <div 
          className="relative w-[90%] md:w-3/5 max-h-3/5 flex flex-col items-center bg-slate-800 border border-2 border-slate-500 rounded-lg overflow-y-auto z-50"
          onClick={(e) => e.stopPropagation()} 
        >
          <div className='absolute top-2 right-2 px-2 bg-fuchsia-900 rounded-full cursor-pointer' onClick={handleClick}>x</div>
          <h1 className='py-4'>Book Some Time!</h1>
          <div className="w-5/6 h-fill flex flex-col items-center text-center">
